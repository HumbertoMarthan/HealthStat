...
public class ScheduleBean extends BeanManagedViewAbstract {
		...
		try {
			if (this.model != null) {
				...
				for (Evento eventoAtual : eventos) { // lista que popula os eventos e inseri
					ScheduleEvent newEvent = new CustomScheduleEvent(eventoAtual.getTitulo(),
							eventoAtual.getDataInicio(), eventoAtual.getDataFim(), eventoAtual.getTipoEvento().getCss(),
							eventoAtual.isDiaInteiro(), eventoAtual.getDescricao(), eventoAtual.getMedico(),
							eventoAtual.getPaciente(), eventoAtual);
	...
	public void sendEmailAgendamento(Evento t) {
		...
		emailConteudo += "<b>DATA/HORA DO INICIO DO AGENDAMENTO :</b> " + DatasUtils.formatDateTime(t.getDataInicio()) + "<br /><br /><br />";
		emailConteudo += "<b>DATA/HORA DO FIM DO AGENDAMENTO :</b> " + DatasUtils.formatDateTime(t.getDataFim()) + "<br /><br /><br />";
	...
	public void buscaEvento() {
		...
		if (campoDataInicio != null) {
			...
			if (campoDataFim != null) {
				aux = DatasUtils.formatDateSql(campoDataFim);
			} else {
				aux = DatasUtils.formatDateSql(DatasUtils.addDays(campoDataFim, 2));
			}
			str.append(" and a.dataInicio BETWEEN '" + DatasUtils.formatDateSql(campoDataInicio) + " 00:00:00' AND '"
					+ aux + " 23:59:59'");
		}
		if (campoDataFim != null) {
			String aux = "";
			if (campoDataInicio != null) {
				aux = DatasUtils.formatDateSql(campoDataInicio);
			} // else { aux = DatasUtils.formatDateSql(campoDataInicio = new Date());}
			str.append(" and a.dataFim BETWEEN '" + aux + " 00:00:00'  AND '" + DatasUtils.formatDateSql(campoDataFim)
					+ " 23:59:59'");
	...
	private List<Map<Object, Object>> lstInfoEstoque;
	Integer estAprovado;
	Integer estReprovado;
	Integer estEmAnalise;
	Integer estAlocaMedico;
	Integer estAlocaEstoque;
	Integer estAlocaRecepcao;
	...
	public void contarEstoque() {
		...
		try {
			str.append("select");
			str.append(" (select count(*) from estoque where status = 'A') as aprovados, ");
			str.append(" (select count(*) from estoque where status = 'R') as reprovados, ");
			str.append(" (select count(*) from estoque where status = 'EM') as emanalise, ");
			str.append(" (select count(*) from estoque where status = 'AL' and setor = 'ME') as alocadosmedico, ");
			str.append(" (select count(*) from estoque where status = 'AL' and setor = 'ES') as alocadosestoque, ");
			str.append(" (select count(*) from estoque where status = 'AL' and setor = 'RC') as alocadosrecepcao ");

			lstInfoEstoque = eventoController.getSqlListMap(str.toString());

			estAprovado = Integer.parseInt(String.valueOf(lstInfoEstoque.get(0).get("aprovados")));
			estReprovado = Integer.parseInt(String.valueOf(lstInfoEstoque.get(0).get("reprovados")));
			estEmAnalise = Integer.parseInt(String.valueOf(lstInfoEstoque.get(0).get("emanalise")));
			estAlocaMedico = Integer.parseInt(String.valueOf(lstInfoEstoque.get(0).get("alocadosmedico")));
			estAlocaEstoque = Integer.parseInt(String.valueOf(lstInfoEstoque.get(0).get("alocadosestoque")));
			estAlocaRecepcao = Integer.parseInt(String.valueOf(lstInfoEstoque.get(0).get("alocadosrecepcao")));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	private List<Map<Object, Object>> lstInfoConsultas;
	Integer resConsulta;
	Integer resRetorno;
	Integer resConfirmacao;
	...
	public void contarConsultas() {
		...
		try {
			str.append("select");
			str.append(" (select count(*) from evento where tipoevento = 0) as consulta, ");
			str.append(" (select count(*) from evento where tipoevento = 1) as retorno, ");
			str.append(" (select count(*) from evento where tipoevento = 2) as confirmacao  ");

			lstInfoConsultas = eventoController.getSqlListMap(str.toString());

			resConsulta = Integer.parseInt(String.valueOf(lstInfoConsultas.get(0).get("consulta")));
			resRetorno = Integer.parseInt(String.valueOf(lstInfoConsultas.get(0).get("retorno")));
			resConfirmacao = Integer.parseInt(String.valueOf(lstInfoConsultas.get(0).get("confirmacao")));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public List<Map<Object, Object>> getLstInfoConsultas() {
		return lstInfoConsultas;
	}

	public void setLstInfoConsultas(List<Map<Object, Object>> lstInfoConsultas) {
		this.lstInfoConsultas = lstInfoConsultas;
	}

	public List<Map<Object, Object>> getLstInfoEstoque() {
		return lstInfoEstoque;
	}

	public void setLstInfoEstoque(List<Map<Object, Object>> lstInfoEstoque) {
		this.lstInfoEstoque = lstInfoEstoque;
	}
	...
	public Integer getEstAprovado() {
		return estAprovado;
	}

	public void setEstAprovado(Integer estAprovado) {
		this.estAprovado = estAprovado;
	}

	public Integer getEstReprovado() {
		return estReprovado;
	}

	public void setEstReprovado(Integer estReprovado) {
		this.estReprovado = estReprovado;
	}

	public Integer getEstEmAnalise() {
		return estEmAnalise;
	}

	public void setEstEmAnalise(Integer estEmAnalise) {
		this.estEmAnalise = estEmAnalise;
	}

	public Integer getEstAlocaMedico() {
		return estAlocaMedico;
	}

	public void setEstAlocaMedico(Integer estAlocaMedico) {
		this.estAlocaMedico = estAlocaMedico;
	}

	public Integer getEstAlocaEstoque() {
		return estAlocaEstoque;
	}

	public void setEstAlocaEstoque(Integer estAlocaEstoque) {
		this.estAlocaEstoque = estAlocaEstoque;
	}

	public Integer getEstAlocaRecepcao() {
		return estAlocaRecepcao;
	}

	public void setEstAlocaRecepcao(Integer estAlocaRecepcao) {
		this.estAlocaRecepcao = estAlocaRecepcao;
	}

	public Integer getResConsulta() {
		return resConsulta;
	}

	public void setResConsulta(Integer resConsulta) {
		this.resConsulta = resConsulta;
	}

	public Integer getResRetorno() {
		return resRetorno;
	}

	public void setResRetorno(Integer resRetorno) {
		this.resRetorno = resRetorno;
	}

	public Integer getResConfirmacao() {
		return resConfirmacao;
	}

	public void setResConfirmacao(Integer resConfirmacao) {
		this.resConfirmacao = resConfirmacao;
	}
